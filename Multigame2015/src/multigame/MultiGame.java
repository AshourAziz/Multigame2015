 ...
 * 
 * MultiGame handles all the timing of the animation to attempt
 * to maintain a consistent frame rate.
 * 
 * The default FPS is 150 frames per second.  
 ...
 * 
 * The MultiGame System can be in any of three different states:
 ...
 * 
 * As MultiGame runs, it reads all the packages in its root directory
 ...
 * 
 * MultiGame will only instantiate a Game if the freePlay variable is true
 * or if the current number of credits is greater than zero.
 * 
 * 
 * </pre>
 */
public class MultiGame extends JFrame implements Runnable {
  ...
  private void gameUpdate() {
    ...
    if (gameState.inState(gameState.SCREENSAVER)) {
      ...
      if ((kh.wasStartBtn1JustPressed())
          ...
          && gameState.getCurTick() > 300) {
        // don't accept clicks until after 2 seconds
        ...
        try {
          ...
          credits -= kh.getCreditsPerGame();
          outFileLog = new EasyWriter(logFileName, "app");
          outFileLog.println("Game Played: " + gameList.get(gameIndex) + " - "
              + new Date());
          outFileLog.close();
          gameState.toState(gameState.INITGAME);
        } catch (Exception e) {
          System.err.println("YOUR GAME MUST HAVE ONLY ONE CONSTRUCTOR\n"
   ...
   * two numbers; - the total frame count, which is the total number of calls to
   * run(); - the frames skipped in this interval, the total number of frames
